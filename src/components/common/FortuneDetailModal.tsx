...
export function FortuneDetailModal({ fortune, onClose }: FortuneDetailModalProps) {
  const intl = useIntl();
  const ActionIcon = ACTION_ICONS[fortune.bestAction] ?? IconSun;
      ...
      <div className="fortune-detail-modal thin-scrollbar">
        <header className="sticky top-0 z-10 flex items-center justify-between border-b border-[var(--border-subtle)] bg-[var(--surface-raised)] px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-[var(--text-faint)]">{fortune.date}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="space-y-6 p-6">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div className={`fortune-indicator ${overallBg(fortune.overall)}`}>
              <IconSun size={20} className={overallColor(fortune.overall)} />
              <span className="text-[10px] uppercase tracking-wider text-[var(--text-faint)]">
                {intl.formatMessage({ id: "fortune.bubble.overall" })}
              </span>
              <span className={`text-[15px] font-bold ${overallColor(fortune.overall)}`}>
                {fortune.overall}
              </span>
            </div>
