...
- **中国 Codex 教育版**（命名打比方：主链路像 Codex 一样什么都能做）：面向学习陪伴与教学编排的独立系统
...
| 版本 | 范围 |
|---|---|
| SPEC v1.0 | 完整设计规格与 MVP 验收标准 |
| MVP | 单学科、完整 1 条任务闭环、L0/L1 为主、L2 异步可选 |
| 能力扩展 | 多学科包、语音完善、教师/监护人授权视图与完整审计证明 |
...
| 角色 | 核心权限 | 边界 |
|---|---|---|
| 学员（初中及以上全学段：初中/高中/大学/成人自学者） | 画像查看/更正、任务执行、反馈、提醒设置 | 高影响计划变更需本人确认 |
| 家长/监护人（未成年学员场景） | 授权范围内周报与风险提示 | 默认不可看私密对话全文 |
| 教师/辅导者 | 聚合学情、授权任务线索 | 最终教育判断由人作出 |
| 管理员 | 账户、内容策略、审计配置 | 不可任意浏览学员内容 |
...
| L2 | 异步数秒~数十秒 | 自研 Agent 多轮 tool call | 周计划、复杂诊断、长解释 | 阻塞输入；自动强制改计划 |
...
与 Codex 式交互范式对齐（主链路打比方：像 Codex 一样什么都能做）：
...
MVP 包含孪生初始化与确认、孪生多次生长可视化、首周计划与动态调整确认、主动启动与线下反馈、L1 规则洞察与干预确认、结构化 Modal、基础任务队列、初中数学学科包及 Demo 数据；手机姿态辅助以授权、类别识别、温和提醒和可选缩时构成可选闭环。

**MVP 实现现状（2026-08-02，实现仓库 `cn-codex-edu`）**：已实现建档向导、三栏主链路、真实 LLM 教练对话、材料登记与页码任务、证据时间戳/周热力图、手机姿态在席检测与定时抽查（含测试）；未实现孪生多次生长、动态计划确认流、拍照错因诊断入库、学科包实体与 Modal-first 教学弹窗协议（逐条对应方案书 §10 实现现状表）。

### 能力完善

语音填窗与 TTS、更完整的知识图谱与错因体系、周期回顾、教师与监护人授权视图，以及稳定的端侧姿态模型和多端孪生同步。

### 能力扩展

多学科包、更丰富且仍坚持最小化原则的线下证据，以及教师协同和完整审计演练能力。
...
| 版本 | 日期 | 说明 |
|---|---|---|
| 1.0 | 2026-07-27 | 整合需求文档与教学引擎设计，形成基础技术规格 |
...
| 1.5 | 2026-08-02 | 目标群体扩展为初中及以上全学段学员；明确"学习态度精神 + 知识掌握能力"双主线孪生；按 GOAI 参赛手册对齐提交物 |
| 1.6 | 2026-08-02 | 与实现仓库 cn-codex-edu 互补核对：补 MVP 实现现状（主链路/姿态管线已实现，孪生生长/动态计划/拍照诊断未实现） |
