...
fn analyze_step_results(
    ...
    for (num, title) in &expected_pairs {
        // 整体成功说明整个流程已走完，未显式打印完成标记的步骤也视为通过。
        let status = if overall_ok {
            StepStatus::Passed
        } else if scan.contains_done(*num) {
            StepStatus::Passed
...
fn strip_step_title_noise(title: &str) -> String {
    // 生成的脚本常是 print("步骤 N：标题", flush=True) 形式；
    // 先去掉 Python 调用的尾部参数再清理装饰符号。
    let core = title.split(',').next().unwrap_or(title);
    // 先循环剥掉引号与右括号，再清掉“—— 完成”和省略号装饰。
    let mut cleaned = core.trim().to_string();
    loop {
...
"#;
    ...
    fn validate_script_id_rejects_path_fragments() {
        ...
        // 非 ASCII id（中文文件名）应被接受；路径片段仍被拒绝。
...
"#;
    ...
    #[tokio::test]
    async fn persist_and_list_reports_roundtrip() {
        let tmp = tempfile::tempdir().unwrap();
        let recordings = tmp.path().join("recordings");
        let scripts_dir = scripts_dir(&recordings);
        tokio::fs::create_dir_all(&scripts_dir).await.unwrap();
        // 提供带步骤清单的脚本，让报告能对比「预期步骤 vs 实际输出」
        tokio::fs::write(
            scripts_dir.join("abc123.py"),
            "# 步骤 1：打开入口页面\n# 步骤 2：输入账号\n# 步骤 3：勾选协议\n",
        )
        .await
        .unwrap();
        ...
        let persisted = persist_last_run(&scripts_dir, "abc123", result).await;
        assert!(persisted.report.is_some());
        let report = persisted.report.as_ref().unwrap();
        assert!(!report.ok);

        // 列举应能找到刚生成的报告
        let listed = list_reports(&recordings, "abc123").await.unwrap();
        assert_eq!(listed.len(), 1);
        assert_eq!(listed[0].id, report.id);

        // 回读报告内容并验证步骤表渲染
        let read = read_report(&recordings, "abc123", &report.id).await.unwrap();
        assert!(read.content.contains("# 回放测试报告"));
        assert!(read.content.contains("## 步骤检查结果"));
        assert!(read.content.contains("✅ 通过"));
        assert!(read.content.contains("❌ 失败"));
        assert!(read.content.contains("⏭️ 未执行"));

        // 删除脚本时应连报告目录一起清理
        delete_script(&recordings, "abc123").await.unwrap();
        let listed_after_delete = list_reports(&recordings, "abc123").await.unwrap();
