...
> 2026-07-23 产品规则更新：小程序功能类型不设限，数据库改为可选能力；所有生成结果必须包含可操作 Web 界面。设置页通过主链路生成后由宿主自动启动，普通主对话直接写入 `codey/miniapps/<slug>` 的合法包也会被自动发现并可在右栏启动。本文后续“数据库必填”“仅业务录入/查询”等描述仅保留为历史方案背景，不再代表当前实现。
